...
def capex_penalty_expr_conv(model, c, y):
    min_conv_threshold = 1000  # MW
    capex_penalty_factor_conv = 1.25
    conv_total = sum(model.X_c[cc, y] for cc in ['PEM', 'Alkaline'])
    # print(f"Year {y}: Conv={value(conv_total)})
    if (value(conv_total) < min_conv_threshold):
        return model.CAPEX_c_base[c, y] * capex_penalty_factor_conv
    else:
        return model.CAPEX_c_base[c, y] * 1.0

def capex_penalty_expr_renw(model, r, y):
    min_renw_threshold = 300  # MW
    capex_penalty_factor_renw = 1.15
    renw_total = sum(model.X_energy[rr, y] for rr in ['PV', 'Wind'])
    # print(f"Year {y}: Renw={value(renw_total)})
    if (value(renw_total) < min_renw_threshold):
        return model.CAPEX_r_base[r, y] * capex_penalty_factor_renw
    else:
        return model.CAPEX_r_base[r, y] * 1.0
...
def run_planning(LCOH_c, LCOE_r, LCOS, Xc_prev=None):
    ...
    model.eta = Param(model.C, initialize={'PEM': 0.7, 'Alkaline': 0.55, 'SMR': 0.75, 'CCUS90': 0.7}) # Efficiency
    model.phi1 = Param(initialize=0.1)  # Discount rate
    model.hours_per_year = Param(initialize=8760)
    grid_share_limit = 0  # Allow max from grid in energy balance planning.
    ...
    #SCENARIO 1 ADITIONAL PARAMETERS
    demand_scale_factor = 0.3  # 30% of original demand

    def interpolate_demand(y):
        demand_values = {2030: 103780000, 2040: 281020000, 2050: 658000000}
        if y < 2030:
            base = demand_values[2030] * (y - 2025) / 5
        elif y < 2040:
            base = demand_values[2030] + (demand_values[2040] - demand_values[2030]) * (y - 2030) / 10
        else:
            base = demand_values[2040] + (demand_values[2050] - demand_values[2040]) * (y - 2040) / 10
        return base * demand_scale_factor  # <-- SCENARIO: Reduced demand

    model.h2_demand = Param(model.Y, initialize=lambda m, y: interpolate_demand(y))

    # CAPEX for conversion technologies USD/MW
    model.CAPEX_c_base = Param(model.C, model.Y, initialize=lambda model, c, y: (
    {('PEM', 2025): 1400000, ('PEM', 2050): 200000, ('Alkaline', 2025): 500000, ('Alkaline', 2050): 150000}
    [c, y] if c in ['PEM', 'Alkaline'] and y in {2025, 2050} else
    {'PEM': 1400000, 'Alkaline': 500000}[c] + (y - 2025) * ({'PEM': (200000 - 1400000), 'Alkaline': (150000 - 500000)}[c] / 25)
    ...
    # CAPEX for resources
    model.CAPEX_r_base = Param(model.R, model.Y, initialize=lambda model, r, y: (
    ...
    def storage_capacity_rule(model, y):
        return model.X_storage[y] >= 0.01 * model.H_annual[y]
    ...
    model.CAPEX_c = Expression(model.C, model.Y, rule=capex_penalty_expr_conv)
    model.CAPEX_r = Expression(model.R, model.Y, rule=capex_penalty_expr_renw)
    CAPEX_info = {}
    ...
    for c in model.C:
        capex_total = 0
        for y in model.Y:
            added_capacity = value(model.X_c[c, y]) - (value(model.X_c[c, y - 1]) if y > 2025 else 0)
    ...
    print("\nCAPEX per Year for Renewable Energy (USD):")
    for r in ['PV', 'Wind']:
        print(f"\n{r} CAPEX:")
        for y in model.Y:
            capex = CAPEX_per_year_renw[r][y]
            print(f"  Year {y}: {capex:,.2f} USD")

    print("\nTotal CAPEX for Renewable Energy (USD):")
    for r in ['PV', 'Wind']:
        print(f"  {r}: {CAPEX_info_renw[r]:,.2f} USD")'''
