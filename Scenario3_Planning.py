...
def run_planning(LCOH_c, LCOE_r, LCOS, Xc_prev=None):
    ...
    {('SMR', 2025): 80000, ('SMR', 2050): 70000, ('CCUS90', 2025): 100000, ('CCUS90', 2050): 100000}
    [c, y] if c in ['SMR', 'CCUS90'] and y in {2025, 2050} else
    (80000 + (y - 2025) * ((70000 - 80000) / 25) if c == 'SMR' else 100000)
    ))
    ...
    def conversion_capacity_rule(model, y):
        return sum(model.X_c[c, y] * model.eta[c] * model.hours_per_year for c in ['PEM', 'Alkaline']) >= model.H_annual[y] * model.E_H2 #* (1 - model.fossil_share[y])
    ...
    #Diversity Share and other params for Scenario 3
    diversity_share = {'PEM': 0.1, 'Alkaline': 0.1, 'SMR': 0.1, 'CCUS90': 0.1}
    model.diversity_share = Param(model.C, initialize=diversity_share)

    def diversity_constraint_rule(model, c, y):
        total_capacity = sum(model.X_c[cc, y] for cc in model.C)
        return model.X_c[c, y] >= model.diversity_share[c] * total_capacity
    model.DiversityConstraint = Constraint(model.C, model.Y, rule=diversity_constraint_rule)
