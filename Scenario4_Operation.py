...
def run_operation(X_c, X_energy, X_storage, X_water, D_y, CAPEX_per_year):
    ...
    for year in sorted(D_y.keys()):
        ...
        def operational_cost_rule(model):
            ...
            carbon_price_per_kg = 0.5  # same value as planning
            smr_carbon_cost = sum(model.H_fossil[t, 'SMR'] * carbon_price_per_kg for t in model.T)
            return energy_costs + conversion_costs + fossil_costs + smr_carbon_cost
