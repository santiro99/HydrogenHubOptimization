...
def run_planning(LCOH_c, LCOE_r, LCOS, Xc_prev=None):
    ...
    grid_share_limit = 0.3  # Allow max from grid in energy balance planning.
    ...
    {('PV', 2025): 200000, ('PV', 2050): 100000, ('Wind', 2025): 400000, ('Wind', 2050): 250000,
    ('Purified Water', 2025): 1.2, ('Purified Water', 2050): 1}
    [r, y] if r in model.R and y in {2025, 2050} else
    {'PV': 300000, 'Wind': 400000, 'Purified Water': 1.2}[r] +
    (y - 2025) * (({'PV': (150000 - 300000), 'Wind': (250000 - 400000), 'Purified Water': (1 - 1.2)}[r]) / 25)
    ))
    ...
    # ---- SCENARIO 2 PARAM: Total wind cap in MW ----
    max_total_wind_cap = 5000
    max_total_pv_cap = 8000

    # Auxiliary vars
    model.WindMaxCap = Var(domain=NonNegativeReals)
    model.PVMaxCap = Var(domain=NonNegativeReals)

    # Limit max across years
    def wind_max_binding_rule(model, y):
        return model.WindMaxCap >= model.X_energy['Wind', y]
    model.WindMaxBind = Constraint(model.Y, rule=wind_max_binding_rule)

    def pv_max_binding_rule(model, y):
        return model.PVMaxCap >= model.X_energy['PV', y]
    model.PVMaxBind = Constraint(model.Y, rule=pv_max_binding_rule)

    # Apply upper limit
    model.TotalWindCap = Constraint(expr=model.WindMaxCap <= max_total_wind_cap)
    model.TotalPVCap = Constraint(expr=model.PVMaxCap <= max_total_pv_cap)

    # ---- Land Penalty Expression ----
    def land_penalty_rule(m):
        land_penalty_pv = 1  # USD/m²
        land_penalty_wind = 5  # USD/m²
        return sum(
            m.X_energy['PV', y] * m.hours_per_year * 12.6 * land_penalty_pv +
            m.X_energy['Wind', y] * m.hours_per_year * 100 * land_penalty_wind
            for y in m.Y
        )

    model.LandPenalty = Expression(rule=land_penalty_rule)

    # Objective function
    def total_cost_rule(model):
        penalty_weight = 10000
        model.Penalty = Expression(expr=sum(model.Dev[c, y] for c in model.C for y in model.Y))
