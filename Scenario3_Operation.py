from pyomo.environ import (ConcreteModel, Var, Objective, Constraint, NonNegativeReals, Expression,
...
def run_operation(X_c, X_energy, X_storage, X_water, D_y, CAPEX_per_year):
    ...
    for year in sorted(D_y.keys()):
        ...
        if result.solver.termination_condition == 'optimal':
            ...
            print(f"\nUtilization Rates for Year {year}:")
            for c in model.C:
                if c in ['PEM', 'Alkaline']:
                    actual = sum(value(model.E[t, c]) * value(model.eta[c]) / value(model.E_H2) for t in model.T)
                    theoretical = X_c[c] * value(model.eta[c]) * value(model.hours_per_year) / value(model.E_H2)
                elif c in ['SMR', 'CCUS90']:
                    actual = sum(value(model.H_fossil[t, c]) for t in model.T)
                    theoretical = X_c[c] * value(model.hours_per_year)
                else:
                    continue
                if theoretical > 0:
                    utilization = actual / theoretical
                    print(f"  {c}: {utilization:.2%}")
                else:
                    print(f"  {c}: No capacity installed.")'''
