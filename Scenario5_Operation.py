...
solverpath_exe='/usr/local/bin/glpsol' #does not need to be directly on c drive
...
def run_operation(X_c, X_energy, X_storage, Xstorage_test, X_water, D_y, CAPEX_per_year):
    ...
    for year in sorted(D_y.keys()):
        ...
        min_storage_fraction = 0.2  # 20% of demand must go through storage
        ...
        if result.solver.termination_condition == 'optimal':
            ...
            print(f"\n[Year {year}] Storage Utilization Metrics:")
    ...
    return total_discounted_opex, opex_per_tech, h2_output_per_tech, grid_opex_per_year, grid_import_year, renewable_energy_yearly, storage_utilization_dict#, total_stored
