...
def run_planning(LCOH_c, LCOE_r, LCOS, Xc_prev=None):
    ...
    # Share of hydrogen demand met by fossil-based techs (SMR, CCUS90) — 26 years from 2025 to 2050
    fossil_share_data = {
        y: 0.9 if y <= 2030 else
        0.4 if y <= 2035 else
        0.2 if y <= 2040 else
        0.1 if y <= 2045 else
        0  # full decarbonization target
    ...
    storage_share_dict = {y: 3e-5 * (y - 2025) for y in range(2025, 2051)}
    model.min_storage_share = Param(model.Y, initialize=storage_share_dict)

    def storage_capacity_rule(model, y):
        return model.X_storage[y] >= model.min_storage_share[y] * model.H_annual[y]
